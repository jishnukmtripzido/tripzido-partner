...
export function VehicleListItem({ vehicle, onClick }: VehicleListItemProps) {
  const statusStyle = vehicle.status
    ? (LISTING_STATUS_STYLES[vehicle.status] ?? "bg-[#F0F4FF] text-[#4A72FF]") // Default to soft blue
    : null;
  const statusLabel = vehicle.status
    ? (LISTING_STATUS_LABELS[vehicle.status] ?? vehicle.status)
    : null;
  ...
  return (
    <button
      onClick={onClick}
      className="w-full bg-white rounded-[1.25rem] p-5 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] border border-gray-50 hover:border-[#FFD166]/60 hover:shadow-md transition-all duration-300 group text-left"
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            ...
            {statusLabel && (
              <span
                className={`shrink-0 text-[10px] font-semibold px-2.5 py-1 rounded-md ${statusStyle}`}
              >
                {statusLabel}
              </span>
            )}
          ...
          <div className="flex items-center gap-2 mt-2">
            <span
              className={`inline-flex items-center text-[11px] font-bold px-2.5 py-1 rounded-md ${
                inStock
                  ? "bg-[#FFF6E0] text-[#D4A33B]" // Soft premium yellow
          ...
          {vehicle.locationName && (
            <p className="text-[12px] text-gray-500 font-medium mt-2.5 flex items-center gap-1.5 truncate">
              <svg
        ...
        <svg
          className="w-5 h-5 text-gray-300 group-hover:text-[#FFD166] transition-colors shrink-0 mt-1"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
