...
export async function getFleetApi(
  ...
  return api.get<FleetPage>(
    `/api/vehicles/vendor/fleet/?${params.toString()}`,
    {
      token: accessToken,
    },
  );
...
// Used by the "Add a bike" wizard as soon as both vehicle_type_id and
// pickup_location_id are picked, so a vendor already having a listing
// for that vehicle type at that pickup location is flagged at
// selection time — instead of only surfacing as a 409 at final
// submit. exclude_listing_id is for the edit-listing flow, so a
// listing being edited doesn't collide with itself.

export interface ListingDuplicateCheckResponse {
  success: boolean;
  message: string;
  data?: { exists: boolean };
}

export async function checkListingDuplicateApi(
  token: string,
  vehicleTypeId: number,
  pickupLocationId: number,
  excludeListingId?: number,
): Promise<ListingDuplicateCheckResponse> {
  const params = new URLSearchParams();
  params.set("vehicle_type_id", String(vehicleTypeId));
  params.set("pickup_location_id", String(pickupLocationId));
  if (excludeListingId)
    params.set("exclude_listing_id", String(excludeListingId));
  return api.get<ListingDuplicateCheckResponse>(
    `/api/vehicles/vendor/fleet/check-duplicate/?${params.toString()}`,
    { token },
  );
}
